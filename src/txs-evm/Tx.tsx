...
import { useEvmAddress, useNetwork } from 'data/wallet';
...
interface Props<TxValues> {
  ...
  createTx: (values: TxValues) => CreateEvmTxOptions | undefined;
...
function Tx<TxValues>(props: Props<TxValues>) {
  ...
  const { data: estimatedGas, ...estimatedGasState } = useQuery(
    ...
    async () => {
      ...
      const msg = simulationTx.msgs[0];

      const gasLimit = await ecd.tx.estimateGas(msg.tx());

      return new BigNumber(gasLimit).times(1).toString();
    },
    {
  ...
  const getGasAmount = useCallback(
    (denom: CoinDenom) => {
      ...
      if (!estimatedGas || isNil(estimatedGas) || !gasPrice) return '0';
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && failed) {
      ...
      console.info(simulationTx?.msgs.map((msg) => (msg)));
      console.info(failed);
      console.groupEnd();
      /* eslint-enable */
    }
  ...
  const submit = async (values: TxValues) => {
    ...
    try {
      ...
      const fee = new Fee(Number(estimatedGas), gasCoins);
