...
const ValidatorActions = ({ destination }: { destination: ValAddress }) => {
  ...
  const renderDelegationsValue = () => {
    ...
    const value = calcValue({ amount, denom: 'axpla' });

    return (
      <section>
        <Read amount={amount} denom="axpla" className={styles.total} block />
        {currency !== 'axpla' && (
          <Read
            amount={value}
            denom={currency}
  ...
  const renderRewardsValue = () => {
    ...
    const amount = list.find(({ denom }) => denom === 'axpla')?.amount ?? '0';

    return (
      <section>
        <Read amount={amount} denom="axpla" className={styles.total} />{' '}
