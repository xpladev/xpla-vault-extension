  ...
  const render = () => {
    ...
    const getProposalDeposited = () => {
      const deposited = deposits.reduce(
        (acc, { amount }) =>
          new BigNumber(acc).plus(getAmount(amount, 'axpla')).toString(),
        '0',
      );

      const minimum = getAmount(depositParams.min_deposit, 'axpla');
    ...
    const contents = [
      {
        title: t('Deposited'),
        content: <Read amount={deposited} denom="axpla" />,
      },
      {
