...
import { ValAddress, Vote } from '@xpla/xpla.js';
import { combineState } from 'data/query';
import { useDelegations } from 'data/queries/staking';
import { useGetVoteOptionItem } from 'data/queries/gov';
import { getCalcVotingPowerRate } from 'data/Xpla/XplaAPI';
import { useXplaProposal, useXplaValidators } from 'data/Xpla/XplaAPI';
...
const ProposalVotesByValidator = ({ id }: { id: number }) => {
  ...
  const { data: XplaProposal, ...XplaProposalState } = useXplaProposal(id);
  const { data: XplaValidators, ...XplaValidatorsState } = useXplaValidators();
  ...
  const getList = useCallback(
    (tab?: Vote.Option) => {
      if (!(delegations && XplaProposal && XplaValidators)) return [];
      ...
      const getHasVoted = (address: ValAddress, option?: Vote.Option) => {
        return XplaProposal.some(({ voter, options }) => {
      ...
      return XplaValidators.filter(({ operator_address, status }) => {
    ...
    [XplaProposal, XplaValidators, delegatedOnly, delegations],
  );
  ...
  const render = () => {
    if (!(delegations && XplaProposal && XplaValidators)) return null;

    const calcRate = getCalcVotingPowerRate(XplaValidators);
  ...
  if (!XplaProposal?.length) return null;
