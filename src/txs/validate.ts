...
const lessThan = (max: BigNumber, label = 'Amount', optional = false) => {
  return (value = '0') => {
    if (optional)
      return !value || max.gte(value) || `${label} must be less than ${max}`;

    return (
      (new BigNumber(value).gt(0) && max.gte(value)) ||
      `${label} must be between 0 and ${max}`
    );
  };
};

const decimal = (decimals = 18, label = 'Amount', optional = false) => {
  return (value = '0') => {
    return (
      (optional && !value) ||
...
const input = (
  ...
  if (maxValue.lte(0)) return always('Insufficient balance');

  return {
    required: (value = '0') => optional || !!value || `${label} is required`,
    lessThan: lessThan(maxValue, label, optional),
