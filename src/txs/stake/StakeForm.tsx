...
const StakeForm = (props: Props) => {
  ...
  const createTx = useCallback(
    ({ input, source }: TxValues) => {
      ...
      const amount = toAmount(input);
      const coin = new Coin('axpla', amount);
  ...
  /* fee */
  const balance = {
    [StakeAction.DELEGATE]: getAmount(balances, 'axpla'),
  ...
  const token = tab === StakeAction.DELEGATE ? 'axpla' : '';
  const tx = {
    token,
    amount,
    balance,
        ...
        <Form onSubmit={handleSubmit(submit.fn)}>
          ...
          >
            <Input
              {...register('input', {
                valueAsNumber: false,
                validate: validate.input(toInput(max.amount, 18), 18),
              })}
              token="axpla"
              onFocus={max.reset}
              inputMode="decimal"
