...
/* contract info */
export const useContractInfo = (address: XplaAddress) => {
...
export const useInitMsg = <T>(address: XplaAddress) => {
  const lcd = useLCDClient();
  return useQuery<T>(
...
/* token info */
export const useTokenInfoCW20 = (token: XplaAddress, enabled = true) => {
  const getQuery = useGetContractQuery();
  return useQuery({
